...
public class UpdateRoomServiceController extends Controller {

    private Stage primaryStage;

    private String previousLocation;

    private RoomService data;
    ...
    public void setData(Object data, String previousLocation){
        ...
        serviceName.setText(this.data.getService());
        servicePrice.setText(this.data.getPrice().toString());
    }

    public void initialize() {
        update.setOnAction(event -> updateRoomService());
        delete.setOnAction(event -> deleteRoomService());
    }

    public void updateRoomService(){
        Alert confirmationAlert = new Alert(Alert.AlertType.CONFIRMATION);
        confirmationAlert.setTitle("Confirm action");
        confirmationAlert.setHeaderText("Are you sure you want to update this room service?");
        ...
        if (result.isPresent() && result.get() == ButtonType.OK) {
            ...
            new Thread(() -> {
                try {
                    ...
                    request.setRef(this.data.getRef());
                    try {
                    ...
                    String response = RestClient.post("/room/service/update", jsonString);
                    ...
                    if (apiResponse.getResponseHeader().getResponseCode().equals("00")) {
                        Platform.runLater(() -> {
                            loadingStage.close();
                            Utils.showAlertDialog(Alert.AlertType.INFORMATION, "Updated Successfully", "Room Service Updated successfully");
                            primaryStage.close();
    ...
    public void deleteRoomService(){
        Alert confirmationAlert = new Alert(Alert.AlertType.CONFIRMATION);
        confirmationAlert.setTitle("Confirm action");
        confirmationAlert.setHeaderText("Are you sure you want to delete this room service?");
        confirmationAlert.setContentText("Please confirm your action.");
        ...
        if (result.isPresent() && result.get() == ButtonType.OK) {
            ...
            new Thread(() -> {
                try {
                    ...
                    String response = RestClient.post("/room/service/delete", jsonString);
                    ...
                    if (apiResponse.getResponseHeader().getResponseCode().equals("00")) {
                        Platform.runLater(() -> {
                            loadingStage.close();
                            Utils.showAlertDialog(Alert.AlertType.INFORMATION, "Deleted Successfully", "Room Service Deleted successfully");
                            primaryStage.close();
