...
public class SingleReservationController extends Controller {
    ...
    public void setData(Object data, String previousLocation){
        ...
        guest_name.setText(this.data.getGuestName());
        phone_number.setText(this.data.getGuestPhoneNumber());
        ...
        for(Room item :this.data.getRoomNumbers()){
                SelectedRoom selectedRoom = new SelectedRoom();
                selectedRoom.setRoomNumber(item.getRoomNumber());
                selectedRoom.setRoomType(item.getRoomType());

                try {
                    String response = RestClient.get("/roomPrices/?roomType=" + item.getRoomType());
...
//                    checkInData.setRoomPrice(roomPrice.getRoomPrice());
                        ...
                        try {
                            ...
                            roomNumberLabel.setText(String.valueOf(item.getRoomNumber()));
                            cancelButton.setOnAction(event -> removeRoom(item.getRoomNumber(), hBox, selectedRoom));

                            rooms_flowPane.getChildren().add(hBox);
                            selectedRooms.add(item.getRoomNumber());
...
//        total_price.setText("₦"+formattedPrice);
    ...
    public void initialize() throws IOException {
        ...
        cancel.setOnAction(event -> cancelReservation());
        ...
        if (result.isPresent() && result.get() == ButtonType.OK) {
            ...
            new Thread(() -> {
                ...
                try {
                    ...
                    String response = RestClient.post("/reservation/update?ref=" + this.data.getRef(), "");
...
//                alert.setHeaderText("This is a header");
    ...
    public void cancelReservation(){
        Alert confirmationAlert = new Alert(Alert.AlertType.CONFIRMATION);
        confirmationAlert.setTitle("Confirm Check-In");
        confirmationAlert.setHeaderText("Are you sure you want to cancel reservation?");
        confirmationAlert.setContentText("Please confirm your action.");
        ...
        if (result.isPresent() && result.get() == ButtonType.OK) {
            ...
            new Thread(() -> {
                ...
                try {
                    ...
                    String response = RestClient.post("/reservation/cancel?ref=" + this.data.getRef(), "");
                    ApiResponseSingleData<?> apiResponse = objectMapper.readValue(response, new TypeReference<ApiResponseSingleData<?>>() {
                    });
                    ...
                    Platform.runLater(() -> {
                        try {
                            if (apiResponse.getResponseHeader().getResponseCode().equals("00")) {
                                ...
                                successAlert.setTitle("Canceled Successful");
                                successAlert.setContentText("The reservation was canceled successfully.");
